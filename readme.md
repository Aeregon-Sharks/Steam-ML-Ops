...
1. **Data Engineering**
    ...
    En esta tarea, se recibieron los conjuntos de datos iniciales: [steam_games.json.gz](Data/steam_games.json.gz), [australian_users_items.json.gz](Data/australian_users_items.json.gz), [australian_user_reviews.json.gz](Data/australian_user_reviews.json.gz), los cuales el catalogo de juegos, el inventario de los usuarios Australianos y sus reseñas respectivamente, estos datos no se subieron al repositorio ya que son muy pesados, deben estar ubicados en la carpeta [Data](Data/) que es la carpeta a la cual apuntan los Notebooks, y deben tener su mismo nombre, puedes descargar estos datos desde aquí: [Datos](https://drive.google.com/drive/folders/1HqBG2-sUkz_R3h1dZU5F2uAzpRn7BSpj). Todo esto se profundiza aún más en la sección de *Instalación y ejecución* y aunque no existan, el Notebook intentará descargarlos automáticamente en tu entorno.<br>
